...
export default function MessageCard(props) {
  ...
  const messageSend = () =>{
    if (titulo===""){
      setErrorMessage("Không thể gửi thư với tiêu đề trống.");
      handleClickAlertError();
    }
    else if (titulo.length>30){
      setErrorMessage("Tiêu đề phải dài tối đa 30 ký tự.");
      handleClickAlertError();
    }
    else if (messagio===""){
      setErrorMessage("Không thể gửi thư với tin nhắn trống.")
      handleClickAlertError();
    }
    else if (messagio.length>4000) {
      setErrorMessage("Tin nhắn bạn đang cố gửi quá lớn(>4000 từ).")
      handleClickAlertError();
    }
    else {
  ...
  return (
    <div style={{marginTop:9, marginLeft:5}}>
      ...
      { props.type==='inbox' &&
        <CardActions>
          <Button size="small" style={{ color: 'teal' }} onClick={()=>{handleOpen()}}><ReplyIcon />&nbsp;Trả lời</Button>
        </CardActions>
      }
    </Card>
    ...
    <Grid container direction={"column"} spacing={1.65}>
      <Grid item>
        ...
        Trả lời {props.message.name} {props.message.surname}
        </Typography>
      </Grid>
      <Grid item>
      ...
      <TextField 
        ...
        <button className='buttonitoMail' onClick={()=>{messageSend()}}>Gửi</button>
    </Box>
    </Modal>
