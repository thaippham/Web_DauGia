...
function PhotoUpload(props) {
    ...
    if (selectedFile) {
        ...
        return (
            <div>
                <Typography sx={{fontFamily: 'Futura'}} variant="h6" >Tên: {selectedFile.name}</Typography>
                <Typography sx={{fontFamily: 'Futura'}} variant="h6" >Loại file: {selectedFile.type}</Typography>
                <Typography sx={{fontFamily: 'Futura'}} variant="h6">
                Sửa đổi lần cuối:{" "}
                    {selectedFile.lastModifiedDate.toDateString()}
                </Typography>
                <button className="buttonitoReverse" onClick={()=>{setSelectedFile(null)}}>Quay lại</button>
